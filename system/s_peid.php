...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if (isset($_SESSION['admin'])) {
    ...
    if ($name != "" AND $des != "" AND $user != "" AND $pass != "" AND $price != "" AND $id  != "") {
    //================================================================
        // <----- preview image ------>
        if(isset($_FILES['file']) AND $_FILES['file']['size'] > 0){
            ...
            if (!in_array($ext, $allowed)) {
                ...
                if (copy($_FILES['file']['tmp_name'],"../../img/box_img/{$basename}")){
                    $basename =$basename;
                    $upt = dd_q("UPDATE id_product SET img =  ? WHERE id = ? ",[$basename,$id]);
                }else{
        ...
        // <----- preview image ------>
        // <----- details image ------>
        $in = dd_q("UPDATE id_product SET user = ? , pass = ? , name = ? , des = ? , price = ? WHERE id = ? ",[
            $user,
            $pass,
            $name,
            $des,
